...
def apply_kinesis_field_mapping(sample_df):
    ...
    projected_df = sample_df.select(
        _optional_column(sample_df, ("partitionKey",), "string").alias("partitionKey"),
        _optional_column(sample_df, ("stream",), "string").alias("stream"),
        _optional_column(sample_df, ("shardId",), "string").alias("shardId"),
        _optional_column(sample_df, ("sequenceNumber",), "string").alias("sequenceNumber"),
        *measurement_columns,
        _optional_column(sample_df, ("Timestamp", "Pi_Timestamp"), "string").alias("Pi_Timestamp"),
        _optional_column(
            sample_df,
            ("approximateArrivalTimestamp", "Kinesis_Timestamp"),
            "timestamp",
        ).alias("Kinesis_Timestamp"),
    )
